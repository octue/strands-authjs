...
export function AuthContextProvider(props) {
  ...
  useEffect(() => {
    ...
    getAuth()
      ...
      .catch((e) => {
        // console.error(e) This is not an error, it just means we're not authenticated right now
        setAuth(false)
      })
    getConfig()
  ...
  return (
    <AuthContext.Provider value={{ auth, config }}>
      {/* TODO REFACTOR REQUEST Children should be able to deal with loading status */}
      {loading ? (
        <Loading />
      ) : auth === false ? (
        <LoadingError />
